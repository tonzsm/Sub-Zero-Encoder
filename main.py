# FILE: Subtitle Embedder v38 (Play Button)
# --- START OF FULL CODE ---
...
class FFmpegGUI:
    def __init__(self, master: TkinterDnD.Tk):
        self.master = master
        master.title("✨ Subtitle Embedder v38 (Play Button) ✨")
        master.geometry("780x780") # ขยายหน้าต่างเล็กน้อย
        ...
        self.last_output_video_path = tk.StringVar() # (v38)
    ...
    def create_widgets(self):
        ...
        button_group_frame = ttk.Frame(action_frame)
        button_group_frame.grid(row=0, column=0, sticky=tk.W)
    ...
    def play_output_video(self):
        """(v38) Plays the last generated video file using the default OS player."""
        video_file = self.last_output_video_path.get()
        if video_file and os.path.exists(video_file):
            try:
                os.startfile(video_file)
            except Exception as e:
                messagebox.showerror("Playback Error", f"Could not play the file:\n\n{e}")
        else:
            messagebox.showwarning("File Not Found", "Output video file not found or process has not been run yet.")
    ...
    def _show_ffmpeg_error_and_quit(self):
        messagebox.showerror("Error", "ffmpeg.exe or ffprobe.exe not found in 'bin' folder!"); self.master.destroy()
    ...
    def update_estimations(self, *args):
        ...
        if total_bps > 0 and self.effective_duration > 0:
            ...
            self.estimated_size_var.set("Est. Size: N/A")
        self.eta_var.set("")
    ...
    def get_video_info(self):
        ...
        try:
            ...
            if 'bit_rate' in format_info:
                ...
                self.output_bitrate.set(f"{round(bitrate_bps / 1000000)}M")
                self.detected_bitrate_str.set(f"{bitrate_bps / 1000:.0f} kbps ({bitrate_bps / 1000000:.2f} Mbps)")
            else:
            ...
            if audio_stream:
                ...
                self.detected_audio_bitrate_bps = int(audio_stream.get('bit_rate', 0))
                audio_bitrate_kbps = self.detected_audio_bitrate_bps // 1000
                channels = int(audio_stream.get('channels', 0))
                channel_str = {1: "mono", 2: "stereo", 6: "5.1"}.get(channels, f"{channels} channels")
                self.detected_audio_str.set(f"{audio_codec.upper()}, {audio_bitrate_kbps} kbps, {channel_str}")
                self.audio_codec_var.set(audio_codec if audio_codec in ['aac', 'ac3'] else 'aac')
                self.audio_bitrate_var.set(f"{audio_bitrate_kbps}k" if audio_bitrate_kbps > 0 else "192k")
            else:
    ...
    def run_ffmpeg_process(self):
        ...
        video_out = os.path.join(output_dir, output_filename)
        self.last_output_folder = output_dir
        self.last_output_video_path.set(video_out)
        ...
        if self.trim_var.get():
            if selected_audio_codec == 'copy':
                selected_audio_codec = 'aac'; self.audio_codec_var.set('aac')
            
            start_s = self.time_to_seconds(self.start_time_var.get())
            end_s = self.time_to_seconds(self.end_time_var.get())
            trim_options = []
            if start_s is not None: trim_options.append(f"start={start_s}")
            if end_s is not None: trim_options.append(f"end={end_s}")
            
            if trim_options:
                trim_str = ":".join(trim_options)
                video_filter_chain.append(f"trim={trim_str}")
                audio_filter_chain.append(f"atrim={trim_str}")
            
            video_filter_chain.append("setpts=PTS-STARTPTS")
            audio_filter_chain.append("asetpts=PTS-STARTPTS")
            ...
            try:
                with open(self.progress_file, 'r', encoding='utf-8') as f:
                    ...
                    if 'out_time_ms' in last_progress and self.effective_duration > 0:
                        processed_s = int(last_progress['out_time_ms']) / 1e6
                        progress = min(processed_s / self.effective_duration * 100, 100.0)
                        ...
                        if progress >= 99.9 and not finishing_signal_sent:
                            ...
                            if progress > 0.5:
                                ...
                                total_time_estimated = elapsed_time / (progress / 100)
                                eta_seconds = total_time_estimated - elapsed_time
        ...
        try:
            while True:
                ...
                if msg_type == "__PROGRESS__":
                    progress_val = message[1]
                    self.progress_var.set(min(progress_val, 99.9)); self.progress_text.set(f"{min(progress_val, 99.9):.1f}%"); self.eta_var.set(message[2])
                elif msg_type == "__FINISHING__":
                    self.progress_var.set(99.9); self.progress_text.set("99.9%"); self.eta_var.set("")
                    self.status_text.set("⏳ Status: Finalizing video file (Muxing)...")
    ...
    def toggle_trim_widgets(self):
        ...
        if self.trim_var.get() and self.audio_codec_var.get() == 'copy':
